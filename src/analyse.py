...
def normalized_overlap_path_equivalence(traj_a_ident, traj_b_ident, trace_a, trace_b, environment, zero_trajectory_threshold=0, impute_val=0):
...
# Map trajectories onto an integer identifier
...
# Take an all_p-like list, desired cell and trajectory. Return a linearized version of the
# cell's firing rate along that trajectory
...
# Return the linearized firing rates for every neighbor trajectory for this cell.
def linearize_neighbor_trajectories(p_env, frequency, cell_num, environment):
    
    all_trajectories = make_trajectory_dict()
...
# Make dict that contains all trajectories and map it to a number
def make_trajectory_dict():
    trajectories = permutations(np.arange(6), r=2)
    trajectory_dict = {}
    for traj_idx, traj in enumerate(trajectories):
...
# Make trajectory dict of only neighbor trajectories
...
# Get vector for all types of path equivalence.
def get_path_equivalence_vector(all_p_env, frequency, cell_num, environment, zero_trajectory_threshold=0, impute_val=0, verbose=False):
    ...
    for trajectory_i, trajectory_a in enumerate(list(trajectory_dict.keys())):
        for trajectory_b in list(trajectory_dict.keys())[trajectory_i:]:
            if (abs(trajectory_a[0] - trajectory_a[1]) == 1) and (abs(trajectory_b[0] - trajectory_b[1]) == 1) and (trajectory_a != trajectory_b):
                ...
                trace_a = linearized_dict[trajectory_a]
                trace_b = linearized_dict[trajectory_b]
                
                traj_a_state_seq = get_trajectory_states(trajectory_a, environment)
                traj_b_state_seq = get_trajectory_states(trajectory_b, environment)
                ...
                normalized_overlap_values.append(normalized_overlap_path_equivalence(trajectory_a, trajectory_b, trace_a, trace_b, environment, zero_trajectory_threshold))
...
# Get vector for all types of path dependence. vec[:5] are flip_direction, vec[5:9] are same_start and
# vec[9:] are same_end
        ...
        for traj_b_ident in neighbor_trajectories[traj_i:]:
            ...
            if (min(traj_a_ident) <= min(traj_b_ident)) and (max(all_arms) - min(all_arms) <= 2) and (traj_a_ident != traj_b_ident):
                ...
                if traj_a_ident == traj_b_ident[::-1]:
                    ...
                    flip_direction.append(normalized_overlap_path_dependence(traj_a_ident, traj_b_ident, trace_a, trace_b, environment, zero_trajectory_threshold, impute_val))
                    ...
                    same_start.append(normalized_overlap_path_dependence(traj_a_ident, traj_b_ident, trace_a, trace_b, environment, zero_trajectory_threshold, impute_val))
                    #if verbose:
                    ...
                    same_end.append(normalized_overlap_path_dependence(traj_a_ident, traj_b_ident, trace_a, trace_b, environment, zero_trajectory_threshold, impute_val))
                    #if verbose:
...
# Return a data matrix of shape (num_cells x num_path_dependence_metrics) 
def get_single_model_all_cells_path_equivalence_matrix(p_env, environment, zero_trajectory_threshold=0):
    
    cell_lst = []

    for FREQUENCY in np.arange(5):
        for cell_num in np.arange(all_p_sweep[0][FREQUENCY][0][0].shape[0]):
            cell_lst.append(get_path_equivalence_vector(all_p_sweep[0], frequency=FREQUENCY, cell_num=cell_num, environment=environment, zero_trajectory_threshold=zero_trajectory_threshold, verbose=False))

    return np.vstack(cell_lst)

# Return a data matrix of shape (num_cells x num_path_dependence_metrics)
def get_single_model_all_cells_path_dependence_matrix(p_env, environment, zero_trajectory_threshold=0):
    
    cell_lst = []

    for FREQUENCY in np.arange(5):
        for cell_num in np.arange(all_p_sweep[0][FREQUENCY][0][0].shape[0]):
            cell_lst.append(get_path_dependence_vector(all_p_sweep[0], frequency=FREQUENCY, cell_num=cell_num, environment=environment, zero_trajectory_threshold=zero_trajectory_threshold, verbose=False))
